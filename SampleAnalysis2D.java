/* SampleAnalysis2D.java */


import java.io.*;
import java.util.*;

public class SampleAnalysis2D {
    
    public static void main(String[] args) {
        
        // Open a new file "SampleAnalysis2D.txt", write the title into the first line of file.
        try {
            FileWriter fw = new FileWriter("2dSampleAnalysis.txt");
    	...
    	for (int i = 0; i <= sampleNumber-1; i++){
    		...
    		double yStart = random.nextDouble() * 50;
       ...
       /*
        *  this part is particular used for function f(x,y) = (1-x)^2 + 100*(y-x^2)^2
        *  the mathematical optimum = 0 where (x, y) = (1, 1);
        *  
        *  for f(x,y) = (x-2)^4 + (x-2y)^2, ; the mathematical optimum = 0 where (x,y) = (2, 1);
        */
       double optimum = 0;
       double sumDis = 0;
       for (int j= 0; j<ff.length; j++) {
